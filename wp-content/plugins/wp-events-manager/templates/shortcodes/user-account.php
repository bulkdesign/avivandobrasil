...
if ( $query->have_posts() ) :
    ...
    <table>
        ...
        <th><?php _e( 'Evento', 'wp-events-manager' ); ?></th>
        <th><?php _e( 'Tipo', 'wp-events-manager' ); ?></th>
        <th><?php _e( 'Preço', 'wp-events-manager' ); ?></th>
        <th><?php _e( 'Quantidade', 'wp-events-manager' ); ?></th>
        <th><?php _e( 'Forma de Pagamento', 'wp-events-manager' ); ?></th>
		...
		<?php foreach ( $query->posts as $post ): ?>
            ...
            <tr>
                ...
                <td><?php printf( '%s', floatval( $booking->price ) == 0 ? __( 'Gratuito', 'wp-events-manager' ) : __( 'Cost', 'wp-events-manager' ) ) ?></td>
                <td><?php printf( '%s', wpems_format_price( floatval( $booking->price ), $booking->currency ) ) ?></td>
                <td><?php printf( '%s', $booking->qty ) ?></td>
                <td><?php printf( '%s', $booking->payment_id ? wpems_get_payment_title( $booking->payment_id ) : __( 'Sem Pagamento', 'wp-events-manager' ) ) ?></td>
	...
	$args = array(
		...
		'mid_size'           => 2,
		'prev_next'          => true,
		'prev_text'          => __( '« Anterior', 'wp-events-manager' ),
		'next_text'          => __( 'Próximo »', 'wp-events-manager' ),
		'type'               => 'plain',
		'add_args'           => false,
		'add_fragment'       => '',
		'before_page_number' => '',
